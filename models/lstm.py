...
class LSTM(nn.Module):
	def __init__(self, args):
		...
		if args.model == 'lstm':
			self.lstm = nn.LSTM(input_size=self.hidden_dim, hidden_size=self.hidden_dim, num_layers=self.stacked_layers, batch_first=True)
			...
			self.lstm = nn.LSTM(input_size=self.hidden_dim, hidden_size=self.hidden_dim, num_layers=self.stacked_layers, bidirectional=True, batch_first=True)
	...
	def forward(self, x):
		...
		if self.model == 'lstm':
			...
			c = torch.zeros((self.stacked_layers, x.size(0), self.hidden_dim), device=self.device)
		else:
			h = torch.zeros((self.stacked_layers*2, x.size(0), self.hidden_dim), device=self.device)
			c = torch.zeros((self.stacked_layers*2, x.size(0), self.hidden_dim), device=self.device)
